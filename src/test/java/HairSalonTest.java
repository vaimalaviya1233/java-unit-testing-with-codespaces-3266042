...
class HairSalonTest {
    ...
    @ParameterizedTest // to pass values in parameter
    @EnumSource(value = HairSalon.Day.class, names= {"MONDAY","TUESDAY"}) //to use enums in tests
    public void testSalonIsOpen(HairSalon.Day day) {
        assertTrue(hairSalon.isOpen(day));
    }

    @ParameterizedTest
    @EnumSource(value = HairSalon.Day.class, names= {"WEDNESDAY","THURSDAY","FRIDAY"})
